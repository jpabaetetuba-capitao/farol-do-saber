const ecologia = [

{
    texto: `Uma professora explicou que a Ecologia busca compreender como os seres vivos interagem entre si e com os componentes físicos do ambiente. Durante a aula, os estudantes analisaram exemplos envolvendo animais, plantas, clima, solo e disponibilidade de água em diferentes regiões.`,

    pergunta: "Considerando o conceito de Ecologia, é correto afirmar que essa área do conhecimento estuda principalmente:",

    alternativas: [

        "as relações estabelecidas entre os seres vivos e entre estes e os fatores presentes no ambiente em que vivem.",

        "a estrutura interna dos organismos e os mecanismos fisiológicos responsáveis pela manutenção da vida.",

        "os processos celulares relacionados à produção de energia e à síntese de substâncias orgânicas.",

        "a classificação dos seres vivos de acordo com suas características evolutivas e genéticas.",

        "a composição química dos ecossistemas e as transformações ocorridas nos ciclos geológicos da Terra."

    ],

    correta: 0,

    feedbackAcerto: "Correto. A Ecologia estuda as interações entre os seres vivos e o ambiente.",

    feedbackErro: "A definição central da Ecologia envolve as relações entre organismos e ambiente.",

    dicaPedagogica: "Palavra-chave: interação."
},

{
    texto: `Em determinado ambiente natural, uma onça caça capivaras, aves alimentam-se de frutos produzidos pelas árvores e peixes competem por alimento em um rio. Essas situações foram utilizadas por um professor para ilustrar exemplos de fenômenos estudados pela Ecologia.`,

    pergunta: "Os exemplos apresentados evidenciam principalmente:",

    alternativas: [

        "interações ecológicas que ocorrem entre organismos e que influenciam a dinâmica dos ecossistemas.",

        "processos fisiológicos responsáveis pela adaptação dos organismos às condições ambientais adversas.",

        "mecanismos evolutivos que promovem o surgimento de novas espécies ao longo do tempo geológico.",

        "transformações químicas relacionadas à circulação de nutrientes nos diferentes ambientes naturais.",

        "fenômenos geográficos responsáveis pela distribuição dos organismos na superfície terrestre."

    ],

    correta: 0,

    feedbackAcerto: "Correto. A Ecologia estuda justamente essas interações observadas na natureza.",

    feedbackErro: "Os exemplos apresentados representam relações ecológicas entre seres vivos.",

    dicaPedagogica: "Predação, competição e alimentação são temas clássicos da Ecologia."
},

{
    texto: `Ao discutir a importância da Ecologia, uma professora destacou que o conhecimento ecológico auxilia na compreensão dos impactos ambientais provocados pelas atividades humanas e na elaboração de estratégias voltadas à conservação dos recursos naturais.`,

    pergunta: "De acordo com essa perspectiva, o estudo da Ecologia é importante porque contribui para:",

    alternativas: [

        "a compreensão dos processos ambientais e a adoção de práticas voltadas ao uso sustentável dos recursos naturais.",

        "o desenvolvimento de técnicas laboratoriais destinadas à manipulação genética dos organismos vivos.",

        "a classificação dos seres vivos segundo critérios taxonômicos internacionalmente reconhecidos.",

        "a elaboração de modelos matemáticos voltados à previsão de eventos geológicos de grande magnitude.",

        "o aprofundamento dos conhecimentos relacionados à estrutura molecular das substâncias presentes na natureza."

    ],

    correta: 0,

    feedbackAcerto: "Correto. A Ecologia possui forte relação com conservação ambiental e sustentabilidade.",

    feedbackErro: "A principal contribuição destacada pela Ecologia está associada à preservação e ao uso sustentável dos recursos naturais.",

    dicaPedagogica: "Conhecer para preservar."
},

{
    texto: `Um estudante afirmou que a Ecologia estuda exclusivamente os seres vivos presentes em determinado ambiente. Outro estudante argumentou que fatores como temperatura, água, solo e luz também fazem parte dos objetos de estudo dessa ciência.`,

    pergunta: "Considerando os fundamentos da Ecologia, a afirmação correta é:",

    alternativas: [

        "o segundo estudante está correto, pois a Ecologia considera tanto fatores bióticos quanto fatores abióticos.",

        "o primeiro estudante está correto, pois os fatores físicos do ambiente pertencem exclusivamente ao campo da Geografia.",

        "ambos estão corretos, pois os fatores ambientais são analisados independentemente dos seres vivos.",

        "nenhum dos estudantes está correto, pois a Ecologia estuda apenas comunidades biológicas complexas.",

        "o primeiro estudante está correto, pois a Ecologia não considera variáveis físicas na análise dos ecossistemas."

    ],

    correta: 0,

    feedbackAcerto: "Correto. Ecologia envolve fatores bióticos e abióticos.",

    feedbackErro: "Os componentes físicos do ambiente fazem parte do estudo ecológico.",

    dicaPedagogica: "Biótico = vivo. Abiótico = não vivo."
},

{
    texto: `Durante uma aula, o professor apresentou a seguinte afirmação: 'A compreensão das relações ecológicas permite prever consequências decorrentes de alterações ambientais provocadas por ações humanas'.`,

    pergunta: "A afirmação apresentada está fundamentada na ideia de que:",

    alternativas: [

        "os ecossistemas são constituídos por elementos interdependentes, de modo que alterações em um componente podem afetar os demais.",

        "as populações naturais apresentam comportamento independente das condições físicas e biológicas do ambiente.",

        "os fatores ambientais exercem influência apenas sobre os organismos produtores presentes nos ecossistemas.",

        "as relações ecológicas possuem importância limitada quando comparadas aos processos fisiológicos dos organismos.",

        "os impactos ambientais provocados pelas atividades humanas não interferem significativamente na dinâmica ecológica."

    ],

    correta: 0,

    feedbackAcerto: "Correto. Os componentes dos ecossistemas são interdependentes.",

    feedbackErro: "Uma alteração ambiental pode desencadear efeitos sobre diversos organismos e processos ecológicos.",

    dicaPedagogica: "Ecossistema = sistema integrado e interdependente."
},
{
    texto: `Em uma reserva ambiental existe uma única onça-pintada que vem sendo monitorada por pesquisadores. Durante uma atividade, os estudantes discutiram em qual nível de organização ecológica esse indivíduo deveria ser classificado.`,

    pergunta: "Considerando os níveis de organização ecológica, a onça descrita corresponde a:",

    alternativas: [

        "um organismo, por representar um indivíduo pertencente a determinada espécie.",

        "uma população, por ocupar uma área delimitada dentro do ecossistema.",

        "uma comunidade, por interagir com outras espécies presentes na reserva.",

        "um ecossistema, por estar associada aos fatores físicos do ambiente.",

        "um bioma, por integrar uma região caracterizada por determinadas condições climáticas."

    ],

    correta: 0,

    feedbackAcerto: "Correto. Organismo corresponde a um único indivíduo.",

    feedbackErro: "População exige vários indivíduos da mesma espécie.",

    dicaPedagogica: "Indivíduo isolado = organismo."
},

{
    texto: `Pesquisadores registraram todos os lobos-guarás existentes em determinada unidade de conservação e passaram a acompanhar o tamanho desse grupo ao longo dos anos.`,

    pergunta: "Nesse caso, o conjunto de indivíduos analisados corresponde a:",

    alternativas: [

        "uma população, pois é formado por indivíduos da mesma espécie vivendo em uma mesma área.",

        "uma comunidade, pois reúne organismos que compartilham o mesmo ambiente físico.",

        "um ecossistema, pois envolve organismos e fatores abióticos interagindo simultaneamente.",

        "um bioma, pois apresenta características ecológicas próprias de determinada região.",

        "uma biosfera, pois representa parte dos seres vivos existentes no planeta."

    ],

    correta: 0,

    feedbackAcerto: "Correto. População = indivíduos da mesma espécie em uma área.",

    feedbackErro: "Comunidade envolve espécies diferentes.",

    dicaPedagogica: "Mesma espécie + mesma área = população."
},

{
    texto: `Em uma lagoa vivem peixes, algas, anfíbios, insetos aquáticos e diversas espécies de plantas. Um estudante afirmou que todos esses organismos, juntos, formam uma comunidade.`,

    pergunta: "A afirmação do estudante está correta porque uma comunidade é formada por:",

    alternativas: [

        "populações de espécies diferentes que vivem e interagem em uma mesma área.",

        "indivíduos da mesma espécie distribuídos em diferentes ambientes naturais.",

        "organismos associados aos fatores físicos responsáveis pela manutenção da vida.",

        "todos os componentes bióticos e abióticos presentes em um ambiente específico.",

        "conjuntos de ecossistemas que compartilham características climáticas semelhantes."

    ],

    correta: 0,

    feedbackAcerto: "Correto. Comunidade é o conjunto de populações de espécies diferentes.",

    feedbackErro: "Ecossistema inclui fatores bióticos e abióticos; comunidade não.",

    dicaPedagogica: "Comunidade = várias populações."
},

{
    texto: `Ao analisar uma floresta, um pesquisador considerou os seres vivos presentes, além de fatores como solo, água, luminosidade, umidade e temperatura.`,

    pergunta: "Considerando os níveis de organização ecológica, o conjunto analisado corresponde a:",

    alternativas: [

        "um ecossistema, por envolver componentes bióticos e abióticos interagindo entre si.",

        "uma comunidade, por reunir populações de diferentes espécies presentes na floresta.",

        "uma população, por representar organismos pertencentes ao mesmo grupo biológico.",

        "um organismo, por representar uma unidade funcional capaz de sobreviver isoladamente.",

        "uma biosfera, por corresponder à totalidade dos ambientes existentes na Terra."

    ],

    correta: 0,

    feedbackAcerto: "Correto. Ecossistema = seres vivos + ambiente físico.",

    feedbackErro: "A presença de fatores abióticos caracteriza um ecossistema.",

    dicaPedagogica: "Biota + ambiente = ecossistema."
},

{
    texto: `Um professor apresentou a seguinte sequência: organismo → população → comunidade → ecossistema → bioma → biosfera.`,

    pergunta: "A sequência apresentada representa:",

    alternativas: [

        "os níveis de organização ecológica organizados do mais simples para o mais abrangente.",

        "as etapas do fluxo de energia observadas nas cadeias alimentares dos ecossistemas.",

        "a classificação dos seres vivos segundo critérios ecológicos e taxonômicos integrados.",

        "os principais processos responsáveis pela formação dos diferentes biomas terrestres.",

        "a hierarquia utilizada para descrever os ciclos biogeoquímicos presentes na biosfera."

    ],

    correta: 0,

    feedbackAcerto: "Correto. Essa é a sequência clássica dos níveis de organização ecológica.",

    feedbackErro: "A questão cobra a hierarquia ecológica do indivíduo até a biosfera.",

    dicaPedagogica: "Organismo → População → Comunidade → Ecossistema → Bioma → Biosfera."
},
{
    texto: `Em determinado ecossistema foi observada a seguinte sequência alimentar:

Capim → Gafanhoto → Sapo → Cobra → Gavião

Durante a análise dessa cadeia alimentar, os estudantes discutiram os papéis desempenhados pelos organismos em cada nível trófico.`,

    pergunta: "Nessa cadeia alimentar, o gafanhoto é classificado como:",

    alternativas: [

        "consumidor primário, pois obtém energia alimentando-se diretamente do produtor.",

        "consumidor secundário, pois participa de uma transferência indireta de energia proveniente dos vegetais.",

        "produtor, pois ocupa posição próxima à base da cadeia alimentar.",

        "decompositor, pois participa da reciclagem da matéria orgânica presente no ambiente.",

        "consumidor terciário, pois serve de alimento para organismos localizados em níveis superiores."

    ],

    correta: 0,

    feedbackAcerto: "Correto. O gafanhoto alimenta-se do produtor, sendo consumidor primário.",

    feedbackErro: "Consumidores primários alimentam-se diretamente dos produtores.",

    dicaPedagogica: "Quem come o produtor é consumidor primário."
},

{
    texto: `Em um campo natural, plantas realizam fotossíntese, herbívoros alimentam-se dessas plantas e carnívoros alimentam-se dos herbívoros. Após a morte dos organismos, fungos e bactérias atuam sobre a matéria orgânica.`,

    pergunta: "A principal função dos decompositores nos ecossistemas é:",

    alternativas: [

        "promover a reciclagem da matéria orgânica, devolvendo nutrientes ao ambiente.",

        "transferir energia solar para os níveis superiores da cadeia alimentar.",

        "produzir matéria orgânica por meio da fotossíntese realizada em diferentes ambientes.",

        "regular o tamanho das populações por meio de relações de predação.",

        "converter compostos inorgânicos em biomassa disponível aos consumidores."

    ],

    correta: 0,

    feedbackAcerto: "Correto. Fungos e bactérias reciclam matéria orgânica e devolvem nutrientes ao ambiente.",

    feedbackErro: "A função central dos decompositores é a reciclagem da matéria.",

    dicaPedagogica: "Decompositores reciclam nutrientes."
},

{
    texto: `Durante uma avaliação, um estudante afirmou que a cadeia alimentar representa apenas uma das possíveis relações alimentares existentes em um ecossistema, enquanto a teia alimentar descreve uma situação mais complexa.`,

    pergunta: "A afirmação está correta porque:",

    alternativas: [

        "a teia alimentar resulta da interligação de várias cadeias alimentares presentes em um mesmo ecossistema.",

        "a cadeia alimentar inclui apenas produtores e consumidores, enquanto a teia alimentar inclui decompositores.",

        "a teia alimentar ocorre exclusivamente em ecossistemas terrestres, enquanto as cadeias ocorrem em todos os ambientes.",

        "a cadeia alimentar representa relações reais, enquanto a teia alimentar representa relações hipotéticas.",

        "a teia alimentar descreve apenas relações ecológicas harmônicas entre os organismos."

    ],

    correta: 0,

    feedbackAcerto: "Correto. A teia alimentar é formada pela integração de diversas cadeias alimentares.",

    feedbackErro: "A principal diferença está na complexidade das relações alimentares.",

    dicaPedagogica: "Cadeia = linear. Teia = várias cadeias interligadas."
},

{
    texto: `Em uma região, o gafanhoto pode servir de alimento para sapos, aves e pequenos mamíferos. Cada um desses organismos, por sua vez, pode integrar diferentes sequências alimentares.`,

    pergunta: "A situação descrita evidencia:",

    alternativas: [

        "a formação de uma teia alimentar, caracterizada por múltiplas relações de alimentação entre os organismos.",

        "a existência de diversas populações ocupando níveis tróficos equivalentes dentro de um único ecossistema.",

        "um exemplo de sucessão ecológica envolvendo organismos pertencentes a diferentes espécies.",

        "a atuação dos decompositores na manutenção do equilíbrio ecológico da região.",

        "um processo de competição interespecífica associado à obtenção de recursos energéticos."

    ],

    correta: 0,

    feedbackAcerto: "Correto. Diversas relações alimentares conectadas formam uma teia alimentar.",

    feedbackErro: "A questão aborda diretamente a definição de teia alimentar.",

    dicaPedagogica: "Quando um organismo participa de várias cadeias, pense em teia alimentar."
},

{
    texto: `Um professor apresentou duas afirmações:

I. A cadeia alimentar apresenta uma sequência linear de transferência de matéria e energia.

II. A teia alimentar representa diversas cadeias interligadas, refletindo melhor a complexidade dos ecossistemas naturais.`,

    pergunta: "Sobre as afirmações apresentadas, assinale a alternativa correta.",

    alternativas: [

        "As afirmações I e II estão corretas, e a II complementa adequadamente a I.",

        "A afirmação I está correta e a II está incorreta, pois teias alimentares não ocorrem naturalmente.",

        "A afirmação I está incorreta e a II está correta, pois cadeias alimentares não representam transferência de energia.",

        "As afirmações I e II estão incorretas, pois matéria e energia circulam da mesma forma nos ecossistemas.",

        "As afirmações I e II estão corretas, porém tratam de fenômenos ecológicos sem qualquer relação entre si."

    ],

    correta: 0,

    feedbackAcerto: "Correto. Cadeia é linear e teia representa a complexidade das relações alimentares.",

    feedbackErro: "As duas afirmações estão corretas e descrevem conceitos complementares.",

    dicaPedagogica: "Pegadinha clássica: cadeia ≠ teia alimentar."
},
{
    texto: `Durante uma aula sobre ecossistemas, um professor afirmou que toda a energia utilizada pelos seres vivos tem origem, direta ou indiretamente, na radiação solar captada pelos produtores.`,

    pergunta: "Considerando o fluxo de energia nos ecossistemas, é correto afirmar que:",

    alternativas: [

        "a energia flui dos produtores para os demais níveis tróficos por meio das relações alimentares estabelecidas entre os organismos.",

        "a energia é continuamente reciclada entre produtores, consumidores e decompositores ao longo do tempo.",

        "os decompositores são responsáveis por devolver toda a energia utilizada pelos organismos ao início da cadeia alimentar.",

        "a quantidade de energia disponível aumenta progressivamente à medida que se avança nos níveis tróficos.",

        "a energia circula em ciclos fechados semelhantes aos observados nos ciclos biogeoquímicos."

    ],

    correta: 0,

    feedbackAcerto: "Correto. A energia entra principalmente pelos produtores e segue pelos níveis tróficos.",

    feedbackErro: "Energia não recicla; ela flui em uma única direção.",

    dicaPedagogica: "Pegadinha clássica: matéria recicla, energia não."
},

{
    texto: `Em uma cadeia alimentar, apenas parte da energia disponível em determinado nível trófico é transferida para o nível seguinte. O restante é utilizado nos processos metabólicos dos organismos e dissipado para o ambiente.`,

    pergunta: "A consequência desse fenômeno é:",

    alternativas: [

        "a redução progressiva da energia disponível nos níveis tróficos mais elevados da cadeia alimentar.",

        "o aumento contínuo da energia disponível para os consumidores localizados no topo da cadeia.",

        "a reciclagem da energia perdida pelos organismos por meio da ação dos decompositores.",

        "a manutenção da mesma quantidade de energia em todos os níveis tróficos existentes no ecossistema.",

        "a transferência integral da energia captada pelos produtores para os consumidores terciários."

    ],

    correta: 0,

    feedbackAcerto: "Correto. A energia diminui ao longo dos níveis tróficos.",

    feedbackErro: "Parte significativa da energia é perdida na forma de calor.",

    dicaPedagogica: "Quanto maior o nível trófico, menor a energia disponível."
},

{
    texto: `Durante uma revisão para concurso, uma professora apresentou as seguintes afirmações:

I. A matéria pode ser reciclada nos ecossistemas.

II. A energia não retorna ao início da cadeia alimentar.

III. O fluxo de energia é unidirecional.`,

    pergunta: "Com base nos conceitos ecológicos, assinale a alternativa correta.",

    alternativas: [

        "As afirmações I, II e III estão corretas.",

        "Apenas as afirmações I e II estão corretas.",

        "Apenas as afirmações II e III estão corretas.",

        "Apenas a afirmação I está correta.",

        "Apenas a afirmação III está correta."

    ],

    correta: 0,

    feedbackAcerto: "Correto. Matéria recicla, energia não recicla e possui fluxo unidirecional.",

    feedbackErro: "Essa é uma das pegadinhas mais frequentes em Ecologia.",

    dicaPedagogica: "Matéria ♻️ recicla. Energia ❌ não recicla."
},

{
    texto: `O ciclo da água garante a circulação contínua desse recurso entre atmosfera, superfície terrestre e organismos vivos. Em determinado momento, a água presente nos oceanos foi aquecida pela radiação solar e passou para o estado gasoso.`,

    pergunta: "O processo descrito corresponde à:",

    alternativas: [

        "evaporação, responsável pela passagem da água líquida para o estado de vapor.",

        "condensação, caracterizada pela formação de nuvens a partir do resfriamento do vapor.",

        "precipitação, relacionada ao retorno da água para a superfície terrestre.",

        "infiltração, associada à penetração da água no solo.",

        "transpiração, processo exclusivo realizado pelos vegetais."

    ],

    correta: 0,

    feedbackAcerto: "Correto. Evaporação é a transformação da água líquida em vapor.",

    feedbackErro: "A questão descreve claramente a ação do calor transformando água líquida em vapor.",

    dicaPedagogica: "Calor solar + água líquida = evaporação."
},

{
    texto: `Após a formação das nuvens, a água retorna à superfície terrestre na forma de chuva, granizo ou neve, contribuindo para a reposição dos recursos hídricos disponíveis nos continentes.`,

    pergunta: "No ciclo da água, o processo descrito é denominado:",

    alternativas: [

        "precipitação, etapa responsável pelo retorno da água da atmosfera para a superfície.",

        "condensação, etapa em que o vapor de água se transforma em nuvens.",

        "evaporação, responsável pela formação inicial do vapor de água atmosférico.",

        "infiltração, caracterizada pela absorção da água pelo solo.",

        "escoamento superficial, associado ao deslocamento da água em direção aos rios."

    ],

    correta: 0,

    feedbackAcerto: "Correto. Precipitação é o retorno da água à superfície.",

    feedbackErro: "Chuva, granizo e neve são formas de precipitação.",

    dicaPedagogica: "Água caindo das nuvens = precipitação."
},
{
    texto: `Uma professora explicou que o ciclo da água é essencial para a manutenção da vida na Terra, pois garante a renovação contínua dos recursos hídricos utilizados pelos seres vivos e pelas atividades humanas.`,

    pergunta: "A importância ecológica do ciclo da água está relacionada principalmente ao fato de que ele:",

    alternativas: [

        "promove a circulação contínua da água entre atmosfera, continentes, oceanos e organismos vivos.",

        "aumenta permanentemente a quantidade total de água existente no planeta por meio da evaporação.",

        "transforma água salgada em água doce durante todas as etapas do processo de circulação hídrica.",

        "impede que a água retorne aos oceanos após sua utilização pelos organismos vivos.",

        "garante que toda a água precipitada seja imediatamente absorvida pelos vegetais."

    ],

    correta: 0,

    feedbackAcerto: "Correto. O ciclo da água mantém sua circulação contínua na natureza.",

    feedbackErro: "A quantidade total de água do planeta permanece praticamente constante.",

    dicaPedagogica: "O ciclo da água promove circulação e renovação dos recursos hídricos."
},

{
    texto: `Durante uma aula sobre ciclos biogeoquímicos, um professor explicou que os vegetais retiram dióxido de carbono da atmosfera para produzir matéria orgânica por meio da fotossíntese.`,

    pergunta: "No ciclo do carbono, a fotossíntese desempenha a função de:",

    alternativas: [

        "remover parte do dióxido de carbono atmosférico e incorporá-lo à matéria orgânica produzida pelos organismos fotossintetizantes.",

        "devolver dióxido de carbono à atmosfera durante a degradação da matéria orgânica realizada pelos decompositores.",

        "promover a liberação de carbono para a atmosfera por meio dos processos respiratórios dos organismos.",

        "converter compostos nitrogenados em moléculas orgânicas utilizadas pelos vegetais.",

        "transformar o carbono presente nos organismos em combustíveis fósseis disponíveis no ambiente."

    ],

    correta: 0,

    feedbackAcerto: "Correto. A fotossíntese retira CO₂ da atmosfera.",

    feedbackErro: "Fotossíntese = captura de carbono atmosférico.",

    dicaPedagogica: "Fotossíntese ↓ CO₂ atmosférico."
},

{
    texto: `Todos os organismos vivos realizam processos metabólicos que resultam na liberação de dióxido de carbono para o ambiente.`,

    pergunta: "No ciclo do carbono, o processo responsável por essa devolução de carbono à atmosfera é:",

    alternativas: [

        "a respiração celular, que libera parte do carbono utilizado pelos organismos.",

        "a fotossíntese, que converte matéria orgânica em dióxido de carbono.",

        "a nitrificação, que transforma compostos nitrogenados em nitratos.",

        "a infiltração, que promove a transferência de carbono para o solo.",

        "a precipitação, que devolve compostos carbonados para a superfície terrestre."

    ],

    correta: 0,

    feedbackAcerto: "Correto. A respiração devolve CO₂ ao ambiente.",

    feedbackErro: "Respiração e fotossíntese possuem funções opostas no ciclo do carbono.",

    dicaPedagogica: "Fotossíntese retira CO₂. Respiração devolve CO₂."
},

{
    texto: `O aumento da concentração de dióxido de carbono atmosférico está associado à intensificação do efeito estufa e ao aquecimento global observado nas últimas décadas.`,

    pergunta: "Entre as atividades humanas abaixo, a que mais contribui diretamente para esse aumento é:",

    alternativas: [

        "a queima de combustíveis fósseis, que libera grandes quantidades de carbono armazenado para a atmosfera.",

        "a infiltração da água da chuva no solo, que favorece a circulação dos nutrientes nos ecossistemas.",

        "a fotossíntese realizada pelos vegetais, responsável pela produção de matéria orgânica.",

        "a decomposição natural da matéria orgânica realizada por fungos e bactérias.",

        "a formação de nuvens durante o processo de condensação do vapor de água."

    ],

    correta: 0,

    feedbackAcerto: "Correto. A queima de combustíveis fósseis é uma das principais fontes antrópicas de CO₂.",

    feedbackErro: "A teoria destaca os combustíveis fósseis como fator importante na intensificação do efeito estufa.",

    dicaPedagogica: "Combustíveis fósseis → mais CO₂ → maior efeito estufa."
},

{
    texto: `Embora o nitrogênio seja o gás mais abundante da atmosfera terrestre, a maioria dos seres vivos não consegue utilizá-lo diretamente na forma gasosa (N₂).`,

    pergunta: "No ciclo do nitrogênio, o processo responsável por transformar o N₂ atmosférico em compostos utilizáveis pelos organismos é denominado:",

    alternativas: [

        "fixação do nitrogênio.",

        "desnitrificação.",

        "assimilação.",

        "decomposição.",

        "respiração."

    ],

    correta: 0,

    feedbackAcerto: "Correto. A fixação converte o N₂ atmosférico em formas utilizáveis.",

    feedbackErro: "A fixação é a porta de entrada do nitrogênio atmosférico nos ecossistemas.",

    dicaPedagogica: "Fixação = transformar N₂ em compostos aproveitáveis."
},
{
    texto: `Durante uma aula sobre ciclos biogeoquímicos, um professor destacou que determinadas bactérias transformam compostos nitrogenados presentes no solo em nitritos e nitratos, formas mais facilmente aproveitadas pelos vegetais.`,

    pergunta: "No ciclo do nitrogênio, o processo descrito corresponde à:",

    alternativas: [

        "nitrificação, responsável pela formação de nitritos e nitratos a partir de compostos nitrogenados.",

        "fixação, responsável pela transformação do nitrogênio atmosférico em compostos utilizáveis.",

        "assimilação, caracterizada pela absorção direta de nitrogênio atmosférico pelos vegetais.",

        "desnitrificação, responsável pelo retorno imediato dos nitratos ao estado sólido.",

        "decomposição, processo que converte matéria orgânica em gás nitrogênio atmosférico."

    ],

    correta: 0,

    feedbackAcerto: "Correto. A nitrificação produz nitritos e nitratos.",

    feedbackErro: "A questão cobra uma das etapas mais importantes do ciclo do nitrogênio.",

    dicaPedagogica: "Nitrificação → nitratos."
},

{
    texto: `Em determinado ecossistema, bactérias presentes no solo transformam nitratos em nitrogênio gasoso, devolvendo esse elemento para a atmosfera.`,

    pergunta: "A etapa descrita corresponde à:",

    alternativas: [

        "desnitrificação, responsável pelo retorno do nitrogênio à atmosfera.",

        "fixação, responsável pela incorporação do nitrogênio atmosférico ao solo.",

        "assimilação, responsável pela absorção de nitratos pelas plantas.",

        "nitrificação, responsável pela produção de nitritos e nitratos.",

        "decomposição, responsável pela degradação da matéria orgânica dos organismos."

    ],

    correta: 0,

    feedbackAcerto: "Correto. A desnitrificação devolve nitrogênio à atmosfera.",

    feedbackErro: "Fixação e desnitrificação possuem funções opostas no ciclo.",

    dicaPedagogica: "Desnitrificação = retorno do nitrogênio para a atmosfera."
},

{
    texto: `Uma abelha coleta néctar de uma flor e, durante esse processo, promove a polinização da planta. Ambos os organismos obtêm benefícios dessa interação.`,

    pergunta: "A relação ecológica descrita é classificada como:",

    alternativas: [

        "mutualismo, pois ambos os organismos envolvidos são beneficiados pela interação.",

        "parasitismo, pois um organismo obtém benefício enquanto o outro sofre prejuízo.",

        "predação, pois um dos organismos depende da eliminação do outro para sobreviver.",

        "competição, pois ambos disputam os mesmos recursos disponíveis no ambiente.",

        "amensalismo, pois um organismo é prejudicado sem que o outro obtenha benefício."

    ],

    correta: 0,

    feedbackAcerto: "Correto. Mutualismo ocorre quando ambos são beneficiados.",

    feedbackErro: "Abelha e flor são exemplos clássicos de mutualismo.",

    dicaPedagogica: "Mutualismo = ganha-ganha."
},

{
    texto: `Carrapatos vivem sobre cães, alimentando-se de seu sangue e podendo causar prejuízos ao hospedeiro.`,

    pergunta: "Essa interação ecológica é classificada como:",

    alternativas: [

        "parasitismo, pois um organismo se beneficia enquanto o outro é prejudicado.",

        "predação, pois ocorre morte imediata do organismo utilizado como alimento.",

        "mutualismo, pois ambos os organismos recebem benefícios da interação.",

        "comensalismo, pois apenas um organismo é beneficiado sem causar prejuízo ao outro.",

        "protocooperação, pois a associação é vantajosa para ambos os participantes."

    ],

    correta: 0,

    feedbackAcerto: "Correto. No parasitismo o parasita beneficia-se e o hospedeiro é prejudicado.",

    feedbackErro: "Parasitismo não envolve benefício para ambos os organismos.",

    dicaPedagogica: "Parasita ganha. Hospedeiro perde."
},

{
    texto: `Uma onça captura e mata uma capivara para alimentar-se. A interação influencia diretamente as populações envolvidas e faz parte da dinâmica natural dos ecossistemas.`,

    pergunta: "A relação ecológica descrita corresponde a:",

    alternativas: [

        "predação, caracterizada pela captura e consumo de uma presa por um predador.",

        "parasitismo, caracterizado pela exploração prolongada de um hospedeiro.",

        "competição, caracterizada pela disputa por recursos ambientais limitados.",

        "mutualismo, caracterizado pelo benefício simultâneo dos organismos envolvidos.",

        "amensalismo, caracterizado pelo prejuízo de um organismo sem benefício ao outro."

    ],

    correta: 0,

    feedbackAcerto: "Correto. Predação envolve captura, morte e consumo da presa.",

    feedbackErro: "A morte da presa é uma característica marcante da predação.",

    dicaPedagogica: "Predador mata e consome a presa."
},
{
    texto: `Duas espécies de aves utilizam os mesmos locais de nidificação e alimentam-se dos mesmos recursos disponíveis em determinada área. Como consequência, a presença de uma espécie influencia negativamente o sucesso da outra.`,

    pergunta: "A relação ecológica descrita é classificada como:",

    alternativas: [

        "competição, pois ocorre disputa por recursos limitados disponíveis no ambiente.",

        "predação, pois uma das espécies elimina a outra para obter alimento.",

        "parasitismo, pois uma espécie obtém benefícios causando prejuízos à outra.",

        "mutualismo, pois ambas as espécies obtêm vantagens decorrentes da interação.",

        "amensalismo, pois apenas uma das espécies é prejudicada pela relação."

    ],

    correta: 0,

    feedbackAcerto: "Correto. Competição ocorre quando organismos disputam recursos limitados.",

    feedbackErro: "A característica central da situação é a disputa pelos mesmos recursos.",

    dicaPedagogica: "Competição = disputa."
},

{
    texto: `Determinados fungos produzem substâncias capazes de inibir o crescimento de bactérias presentes no ambiente, sem que os próprios fungos obtenham benefício direto dessa interação.`,

    pergunta: "A relação ecológica descrita corresponde a:",

    alternativas: [

        "amensalismo, pois um organismo é prejudicado enquanto o outro não é afetado.",

        "parasitismo, pois o fungo utiliza recursos fornecidos pelas bactérias.",

        "competição, pois fungos e bactérias disputam exatamente os mesmos recursos.",

        "mutualismo, pois ambos os organismos recebem benefícios da interação.",

        "protocooperação, pois a associação favorece os dois organismos envolvidos."

    ],

    correta: 0,

    feedbackAcerto: "Correto. No amensalismo um é prejudicado e o outro permanece indiferente.",

    feedbackErro: "O fungo não recebe benefício direto, apenas a bactéria sofre prejuízo.",

    dicaPedagogica: "Amensalismo = um perde, outro indiferente."
},

{
    texto: `Após uma erupção vulcânica, uma área ficou totalmente desprovida de solo e de organismos vivos. Com o passar do tempo, líquens começaram a colonizar a região, iniciando o processo de ocupação biológica.`,

    pergunta: "A situação descrita representa um exemplo de:",

    alternativas: [

        "sucessão ecológica primária, pois ocorre em uma área sem solo previamente desenvolvido.",

        "sucessão ecológica secundária, pois ocorre em uma área anteriormente ocupada por uma comunidade.",

        "sucessão ecológica acelerada, pois há participação direta da atividade humana.",

        "competição interespecífica, pois diferentes organismos disputam o mesmo espaço físico.",

        "amensalismo ecológico, pois os organismos pioneiros modificam o ambiente para outras espécies."

    ],

    correta: 0,

    feedbackAcerto: "Correto. Sucessão primária ocorre em áreas sem solo e sem vida anterior.",

    feedbackErro: "A ausência de solo é a principal pista para identificar sucessão primária.",

    dicaPedagogica: "Primária = sem solo."
},

{
    texto: `Uma floresta sofreu um incêndio, mas o solo permaneceu preservado. Nos anos seguintes, diferentes espécies voltaram a ocupar gradualmente a área afetada.`,

    pergunta: "O processo descrito corresponde a:",

    alternativas: [

        "sucessão ecológica secundária, pois ocorre em uma área anteriormente ocupada e que ainda possui solo.",

        "sucessão ecológica primária, pois toda sucessão iniciada após um impacto ambiental recebe essa classificação.",

        "amensalismo, pois algumas espécies dificultam o crescimento de outras durante a recolonização.",

        "predação ecológica, pois ocorre substituição progressiva das espécies presentes no ambiente.",

        "mutualismo sucessional, pois as espécies colaboram para reconstruir a comunidade."

    ],

    correta: 0,

    feedbackAcerto: "Correto. A presença de solo caracteriza sucessão secundária.",

    feedbackErro: "Primária = sem solo. Secundária = solo já existente.",

    dicaPedagogica: "Pegadinha clássica de concurso."
},

{
    texto: `Durante o estudo dos biomas brasileiros, um estudante afirmou que determinado bioma é considerado a maior floresta tropical do planeta e apresenta uma das maiores biodiversidades do mundo.`,

    pergunta: "O bioma descrito é:",

    alternativas: [

        "Amazônia.",

        "Cerrado.",

        "Caatinga.",

        "Pantanal.",

        "Pampa."

    ],

    correta: 0,

    feedbackAcerto: "Correto. A Amazônia é a maior floresta tropical do mundo.",

    feedbackErro: "A descrição corresponde claramente ao bioma Amazônia.",

    dicaPedagogica: "Maior floresta tropical = Amazônia."
},
{
    texto: `Uma professora explicou que a biodiversidade não se refere apenas à quantidade de espécies existentes em determinada região, mas também à variedade genética e à diversidade de ecossistemas presentes no ambiente.`,

    pergunta: "Com base nesse conceito, a biodiversidade pode ser compreendida como:",

    alternativas: [

        "a variedade de formas de vida, incluindo diversidade genética, de espécies e de ecossistemas.",

        "o número total de indivíduos pertencentes à espécie mais abundante de um ecossistema.",

        "a quantidade de matéria orgânica produzida pelos organismos fotossintetizantes de uma região.",

        "o conjunto de fatores físicos responsáveis pela manutenção dos ecossistemas terrestres.",

        "a distribuição geográfica das populações humanas nos diferentes biomas do planeta."

    ],

    correta: 0,

    feedbackAcerto: "Correto. Biodiversidade envolve genes, espécies e ecossistemas.",

    feedbackErro: "Biodiversidade é um conceito amplo e não se limita ao número de espécies.",

    dicaPedagogica: "Biodiversidade = genes + espécies + ecossistemas."
},

{
    texto: `Durante uma discussão sobre recursos naturais, um estudante citou a energia solar, os ventos e a água dos rios como exemplos de recursos que podem ser utilizados continuamente sem esgotamento imediato.`,

    pergunta: "Os recursos mencionados são classificados como:",

    alternativas: [

        "renováveis, pois podem ser repostos naturalmente em escalas de tempo compatíveis com sua utilização.",

        "não renováveis, pois dependem de processos geológicos extremamente lentos para sua reposição.",

        "esgotáveis, pois desaparecem definitivamente após serem utilizados pelos seres humanos.",

        "minerais, pois possuem origem exclusivamente associada à dinâmica geológica da Terra.",

        "biológicos, pois são produzidos diretamente pelos organismos vivos presentes nos ecossistemas."

    ],

    correta: 0,

    feedbackAcerto: "Correto. Sol, vento e água são exemplos clássicos de recursos renováveis.",

    feedbackErro: "Recursos renováveis possuem reposição natural relativamente rápida.",

    dicaPedagogica: "Solar, eólica e hídrica = renováveis."
},

{
    texto: `O lançamento de resíduos industriais sem tratamento adequado em rios pode provocar alterações na qualidade da água, afetando organismos aquáticos e comprometendo o equilíbrio dos ecossistemas.`,

    pergunta: "A situação descrita representa um exemplo de:",

    alternativas: [

        "impacto ambiental decorrente da ação humana sobre os ecossistemas naturais.",

        "sucessão ecológica secundária associada à modificação dos recursos hídricos.",

        "relação ecológica desarmônica entre organismos pertencentes a diferentes espécies.",

        "ciclo biogeoquímico responsável pela redistribuição dos nutrientes nos ambientes aquáticos.",

        "processo natural de renovação dos ecossistemas aquáticos ao longo do tempo."

    ],

    correta: 0,

    feedbackAcerto: "Correto. A poluição hídrica é um importante impacto ambiental.",

    feedbackErro: "O enunciado descreve claramente uma alteração ambiental causada por atividades humanas.",

    dicaPedagogica: "Poluição = impacto ambiental."
},

{
    texto: `Uma comunidade adotou medidas como redução do desperdício de água, reciclagem de resíduos, utilização de fontes renováveis de energia e incentivo ao consumo consciente.`,

    pergunta: "As ações descritas estão diretamente relacionadas ao conceito de:",

    alternativas: [

        "sustentabilidade, baseada na utilização responsável dos recursos naturais visando atender às necessidades atuais sem comprometer as futuras gerações.",

        "preservação absoluta, fundamentada na proibição total do uso dos recursos naturais disponíveis no ambiente.",

        "sucessão ecológica, caracterizada pela substituição gradual das comunidades presentes em um ecossistema.",

        "competição ecológica, associada à disputa por recursos limitados entre diferentes populações.",

        "biodiversidade, relacionada à variedade de organismos encontrados em uma determinada região."

    ],

    correta: 0,

    feedbackAcerto: "Correto. Sustentabilidade envolve equilíbrio entre uso dos recursos e conservação ambiental.",

    feedbackErro: "Sustentabilidade não significa deixar de usar os recursos naturais, mas utilizá-los de forma responsável.",

    dicaPedagogica: "Pegadinha: sustentabilidade ≠ preservação absoluta."
},

{
    texto: `Pesquisadores observaram aumento da temperatura média global, alterações nos regimes de chuva, intensificação de eventos climáticos extremos e elevação do nível médio dos oceanos em diferentes regiões do planeta.`,

    pergunta: "Esses fenômenos estão associados principalmente:",

    alternativas: [

        "às mudanças climáticas relacionadas à intensificação do efeito estufa provocada pelo aumento das concentrações de gases na atmosfera.",

        "à sucessão ecológica observada nos ecossistemas terrestres após perturbações ambientais de grande escala.",

        "ao aumento natural da biodiversidade decorrente da expansão dos ecossistemas tropicais em diferentes continentes.",

        "à redução dos processos respiratórios realizados pelos organismos presentes nos ecossistemas naturais.",

        "à interrupção dos ciclos biogeoquímicos responsáveis pela circulação da matéria na biosfera."

    ],

    correta: 0,

    feedbackAcerto: "Correto. O aumento dos gases de efeito estufa está diretamente relacionado às mudanças climáticas atuais.",

    feedbackErro: "A intensificação do efeito estufa é um dos principais fatores associados ao aquecimento global.",

    dicaPedagogica: "Mudanças climáticas ↔ aumento dos gases de efeito estufa."
}

];