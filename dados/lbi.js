const lbi = [

{
    pergunta: "Durante um debate sobre inclusão, um servidor afirmou que a caracterização da pessoa com deficiência depende exclusivamente da existência de uma limitação física ou mental. Outro participante destacou que a legislação adota conceito mais amplo. De acordo com a Lei Brasileira de Inclusão, assinale a alternativa correta:",

    alternativas: [
        "A deficiência é definida exclusivamente por diagnóstico médico.",
        "A deficiência corresponde apenas às limitações físicas permanentes.",
        "A deficiência depende exclusivamente da avaliação da capacidade laboral da pessoa.",
        "A deficiência resulta da interação entre impedimentos de longo prazo e barreiras que podem obstruir a participação plena e efetiva na sociedade.",
        "A deficiência somente existe quando impede totalmente a autonomia do indivíduo."
    ],

    correta: 3,

    feedbackAcerto: "Correto. A LBI adota modelo biopsicossocial, entendendo a deficiência como resultado da interação entre impedimentos de longo prazo e barreiras presentes na sociedade.",

    feedbackErro: "A legislação não adota visão exclusivamente médica. A deficiência é compreendida a partir da interação entre características da pessoa e as barreiras existentes.",

    dicaBanca: "Palavra-chave: interação entre impedimentos e barreiras."
},
{
    pergunta: "Ao analisar um pedido de reconhecimento da condição de pessoa com deficiência, uma equipe técnica discutiu quais critérios deveriam ser considerados. Segundo a LBI, a avaliação da deficiência deve:",

    alternativas: [
        "Basear-se exclusivamente em laudo médico.",
        "Considerar apenas limitações físicas observáveis.",
        "Ser biopsicossocial, realizada por equipe multiprofissional e interdisciplinar.",
        "Ser realizada exclusivamente por assistente social.",
        "Depender apenas da existência de incapacidade para o trabalho."
    ],

    correta: 2,

    feedbackAcerto: "Correto. A LBI determina avaliação biopsicossocial realizada por equipe multiprofissional e interdisciplinar.",

    feedbackErro: "A legislação superou a avaliação exclusivamente médica e passou a adotar abordagem biopsicossocial.",

    dicaBanca: "Decore a expressão: equipe multiprofissional e interdisciplinar."
},
{
    pergunta: "Uma prefeitura pretende construir um novo centro cultural e decidiu incorporar recursos de acessibilidade já na fase de elaboração do projeto, evitando adaptações futuras. Essa medida está alinhada ao conceito de:",

    alternativas: [
        "Atendimento prioritário.",
        "Desenho universal.",
        "Tecnologia assistiva.",
        "Curatela preventiva.",
        "Tomada de decisão apoiada."
    ],

    correta: 1,

    feedbackAcerto: "Correto. O desenho universal busca conceber ambientes, produtos e serviços utilizáveis pelo maior número possível de pessoas desde sua origem.",

    feedbackErro: "O desenho universal procura eliminar barreiras já na fase de planejamento, reduzindo a necessidade de adaptações posteriores.",

    dicaBanca: "Desenho universal = acessibilidade pensada desde o início."
},{
    pergunta: "Durante a elaboração de um projeto arquitetônico, um engenheiro afirmou que desenho universal e adaptação razoável possuem exatamente o mesmo significado. À luz da LBI, assinale a alternativa correta:",

    alternativas: [
        "A afirmação está correta.",
        "Desenho universal e adaptação razoável são conceitos equivalentes.",
        "A adaptação razoável substitui o desenho universal.",
        "A adaptação razoável somente pode ser aplicada em escolas.",
        "O desenho universal busca atender ao maior número de pessoas desde a concepção, enquanto a adaptação razoável consiste em ajustes específicos quando necessários."
    ],

    correta: 4,

    feedbackAcerto: "Correto. O desenho universal atua preventivamente no projeto, enquanto a adaptação razoável envolve ajustes individualizados para garantir direitos.",

    feedbackErro: "Embora relacionados à inclusão, os dois conceitos possuem finalidades distintas na legislação.",

    dicaBanca: "Desenho universal = planejamento; adaptação razoável = ajuste específico."
},{
    pergunta: "Uma candidata afirmou que somente pessoas com deficiência física podem ser reconhecidas como pessoas com deficiência para fins de proteção da LBI. Considerando a legislação, assinale a alternativa correta:",

    alternativas: [
        "A legislação considera diferentes tipos de impedimentos de longo prazo, inclusive físicos, mentais, intelectuais e sensoriais.",
        "A afirmação está correta.",
        "A proteção alcança apenas deficiências motoras.",
        "A legislação protege apenas deficiências visíveis.",
        "A condição de pessoa com deficiência depende exclusivamente da incapacidade laboral."
    ],

    correta: 0,

    feedbackAcerto: "Correto. A LBI contempla impedimentos físicos, mentais, intelectuais e sensoriais que, em interação com barreiras, possam limitar a participação social.",

    feedbackErro: "A legislação possui conceito amplo e não se restringe às deficiências físicas.",

    dicaBanca: "Lembre-se dos quatro grupos: físico, mental, intelectual e sensorial."
},{
    pergunta: "Ao analisar um pedido de reconhecimento da condição de pessoa com deficiência, uma comissão discutiu quais critérios deveriam ser utilizados na avaliação. De acordo com a Lei Brasileira de Inclusão, a avaliação da deficiência, quando necessária, deve ser:",

    alternativas: [
        "Biopsicossocial, realizada por equipe multiprofissional e interdisciplinar.",
        "Exclusivamente médica, baseada no diagnóstico clínico.",
        "Realizada apenas por profissional da área da saúde.",
        "Fundamentada apenas na existência de limitações físicas.",
        "Baseada exclusivamente na capacidade laboral da pessoa."
    ],

    correta: 0,

    feedbackAcerto: "Correto. A LBI determina que a avaliação da deficiência seja biopsicossocial e realizada por equipe multiprofissional e interdisciplinar.",

    feedbackErro: "A legislação adota abordagem ampla, superando a análise exclusivamente médica da deficiência.",

    dicaBanca: "Memorize a expressão: equipe multiprofissional e interdisciplinar."
},{
    pergunta: "Durante um curso sobre inclusão, um participante afirmou que a avaliação da deficiência deve considerar apenas laudos médicos e exames clínicos. À luz da LBI, assinale a alternativa correta:",

    alternativas: [
        "A afirmação está correta.",
        "A avaliação limita-se às condições de saúde da pessoa.",
        "Somente exames médicos possuem valor legal.",
        "A avaliação deve considerar fatores corporais, socioambientais, psicológicos e pessoais.",
        "A análise restringe-se ao tipo de deficiência apresentado."
    ],

    correta: 3,

    feedbackAcerto: "Correto. A avaliação biopsicossocial considera diversos fatores que influenciam a participação da pessoa na sociedade.",

    feedbackErro: "A LBI adota uma visão ampla da deficiência e não se limita ao diagnóstico médico.",

    dicaBanca: "A deficiência é analisada em conjunto com fatores do ambiente e da participação social."
},{
    pergunta: "Uma equipe responsável pela avaliação biopsicossocial discutia os elementos que deveriam compor sua análise. Segundo a Lei Brasileira de Inclusão, um dos aspectos obrigatoriamente considerados é:",

    alternativas: [
        "A renda familiar da pessoa avaliada.",
        "A opinião exclusiva do médico responsável.",
        "O histórico escolar dos familiares.",
        "A situação patrimonial do interessado.",
        "A funcionalidade e as restrições de participação da pessoa."
    ],

    correta: 4,

    feedbackAcerto: "Correto. A funcionalidade e as restrições de participação constituem elementos expressamente considerados na avaliação biopsicossocial.",

    feedbackErro: "A avaliação busca compreender como a pessoa interage com o ambiente e quais barreiras afetam sua participação social.",

    dicaBanca: "Funcionalidade e participação aparecem frequentemente na legislação."
},{
    pergunta: "Uma candidata afirmou que a existência de uma deficiência deve ser determinada exclusivamente pela presença de impedimentos físicos ou sensoriais. De acordo com a LBI, essa afirmação é:",

    alternativas: [
        "Correta, pois a deficiência depende apenas das limitações corporais.",
        "Correta apenas nos casos de deficiência permanente.",
        "Incorreta, pois a avaliação também considera barreiras que dificultam a participação social.",
        "Incorreta apenas para deficiência intelectual.",
        "Correta quando houver laudo médico definitivo."
    ],

    correta: 2,

    feedbackAcerto: "Correto. A deficiência resulta da interação entre impedimentos e barreiras que podem restringir a participação plena e efetiva na sociedade.",

    feedbackErro: "A legislação não adota uma visão exclusivamente biológica da deficiência.",

    dicaBanca: "A palavra-chave é interação entre impedimentos e barreiras."
},{
    pergunta: "Durante um processo de avaliação, verificou-se que uma pessoa possuía impedimento de longo prazo, mas encontrava poucas barreiras em seu ambiente de convivência. Segundo a lógica adotada pela LBI, a análise dessa situação deve:",

    alternativas: [
        "Desconsiderar totalmente as condições do ambiente.",
        "Levar em conta tanto os impedimentos quanto os fatores ambientais e sociais.",
        "Basear-se exclusivamente no laudo médico.",
        "Considerar apenas a gravidade do impedimento.",
        "Ignorar aspectos relacionados à participação social."
    ],

    correta: 1,

    feedbackAcerto: "Correto. A avaliação biopsicossocial exige análise conjunta dos impedimentos e dos fatores ambientais que influenciam a participação da pessoa.",

    feedbackErro: "A legislação adota modelo que considera a interação entre a pessoa e o ambiente em que vive.",

    dicaBanca: "O ambiente pode ampliar ou reduzir os efeitos das barreiras."
},{
    pergunta: "Durante a reforma de um prédio público, foram instaladas rampas, elevadores adaptados e sinalização tátil. Um gestor afirmou que tais medidas esgotam o conceito de acessibilidade previsto na Lei Brasileira de Inclusão. Assinale a alternativa correta:",

    alternativas: [
        "A acessibilidade limita-se à eliminação de barreiras arquitetônicas.",
        "A acessibilidade refere-se apenas às pessoas com deficiência física.",
        "A acessibilidade aplica-se exclusivamente aos órgãos públicos.",
        "A acessibilidade é obrigatória apenas em construções novas.",
        "A acessibilidade compreende a eliminação de barreiras físicas, comunicacionais, tecnológicas, informacionais e atitudinais."
    ],

    correta: 4,

    feedbackAcerto: "Correto. A LBI adota conceito amplo de acessibilidade, abrangendo diferentes tipos de barreiras que podem limitar a participação social.",

    feedbackErro: "A acessibilidade vai muito além de rampas e elevadores, alcançando comunicação, informação, tecnologia e atitudes.",

    dicaBanca: "Acessibilidade não é apenas física."
},
{
    pergunta: "Uma pessoa com deficiência visual não consegue utilizar determinado portal eletrônico porque as informações não são compatíveis com leitores de tela. Nessa situação, o principal obstáculo identificado é uma barreira:",

    alternativas: [
        "Arquitetônica.",
        "Urbanística.",
        "Nos transportes.",
        "Tecnológica e de comunicação.",
        "Natural."
    ],

    correta: 3,

    feedbackAcerto: "Correto. A impossibilidade de acesso a sistemas digitais acessíveis caracteriza barreira tecnológica e comunicacional.",

    feedbackErro: "A legislação reconhece que barreiras podem existir também no ambiente virtual e tecnológico.",

    dicaBanca: "Sites inacessíveis costumam caracterizar barreiras tecnológicas."
},{
    pergunta: "Em determinado órgão público, servidores passaram a evitar o atendimento direto a pessoas com deficiência por acreditarem que elas não compreenderiam as informações prestadas. Segundo a LBI, essa situação caracteriza principalmente uma barreira:",

    alternativas: [
        "Arquitetônica.",
        "Atitudinal.",
        "Urbanística.",
        "Tecnológica.",
        "Instrumental."
    ],

    correta: 1,

    feedbackAcerto: "Correto. Barreiras atitudinais decorrem de preconceitos, estigmas e comportamentos discriminatórios.",

    feedbackErro: "A legislação reconhece que atitudes preconceituosas também impedem a inclusão social.",

    dicaBanca: "Preconceito = barreira atitudinal."
},{
    pergunta: "Uma prefeitura realizou obras para garantir circulação segura de pessoas com deficiência em calçadas, praças e vias públicas. O objetivo principal foi eliminar:",

    alternativas: [
        "Barreiras tecnológicas.",
        "Barreiras pedagógicas.",
        "Barreiras urbanísticas.",
        "Barreiras administrativas.",
        "Barreiras processuais."
    ],

    correta: 2,

    feedbackAcerto: "Correto. Barreiras urbanísticas são aquelas presentes em vias públicas, espaços de uso coletivo e áreas urbanas.",

    feedbackErro: "Calçadas, praças e vias públicas estão diretamente relacionadas às barreiras urbanísticas.",

    dicaBanca: "Urbanística = cidade e espaços públicos."
},{
    pergunta: "Uma empresa instalou plataforma elevatória para acesso ao interior de seu edifício, eliminando obstáculos que dificultavam a entrada de pessoas com mobilidade reduzida. A medida destinou-se principalmente à remoção de:",

    alternativas: [
        "Barreiras arquitetônicas.",
        "Barreiras comunicacionais.",
        "Barreiras informacionais.",
        "Barreiras atitudinais.",
        "Barreiras tecnológicas."
    ],

    correta: 0,

    feedbackAcerto: "Correto. Barreiras arquitetônicas são aquelas existentes em edificações públicas ou privadas.",

    feedbackErro: "Obstáculos relacionados ao acesso físico em edificações caracterizam barreiras arquitetônicas.",

    dicaBanca: "Prédio = barreira arquitetônica."
},{
    pergunta: "Ao planejar a construção de um novo terminal rodoviário, a equipe técnica decidiu incorporar recursos de acessibilidade desde a fase de elaboração do projeto, buscando atender ao maior número possível de usuários sem necessidade de adaptações futuras. Essa medida está alinhada ao conceito de:",

    alternativas: [
        "Desenho universal.",
        "Adaptação razoável.",
        "Tecnologia assistiva.",
        "Acessibilidade corretiva.",
        "Atendimento prioritário."
    ],

    correta: 0,

    feedbackAcerto: "Correto. O desenho universal consiste na concepção de produtos, ambientes, programas e serviços utilizáveis pelo maior número possível de pessoas desde sua origem.",

    feedbackErro: "O desenho universal busca eliminar barreiras já na fase de planejamento, reduzindo a necessidade de adaptações posteriores.",

    dicaBanca: "Desenho universal = acessibilidade pensada desde o início."
},

{
    pergunta: "Uma universidade passou a disponibilizar provas em formato ampliado para uma estudante com baixa visão, garantindo que ela realizasse a avaliação em igualdade de condições com os demais alunos. Essa medida constitui exemplo de:",

    alternativas: [
        "Barreira urbanística.",
        "Adaptação razoável.",
        "Curatela educacional.",
        "Atendimento preferencial.",
        "Desenho universal."
    ],

    correta: 1,

    feedbackAcerto: "Correto. Adaptação razoável corresponde a modificações e ajustes necessários para assegurar o exercício de direitos em igualdade de condições.",

    feedbackErro: "A adaptação razoável consiste em ajustes específicos realizados para atender necessidades concretas de determinada pessoa.",

    dicaBanca: "Adaptação razoável = ajuste individualizado."
},

{
    pergunta: "Durante um seminário sobre inclusão, um participante afirmou que desenho universal e adaptação razoável possuem exatamente o mesmo significado. À luz da LBI, essa afirmação é:",

    alternativas: [
        "Correta, pois ambos os conceitos são sinônimos.",
        "Correta apenas em ambientes escolares.",
        "Incorreta apenas para órgãos públicos.",
        "Correta quando houver pessoa com deficiência física.",
        "Incorreta, pois o desenho universal atua na concepção inicial e a adaptação razoável consiste em ajustes específicos quando necessários."
    ],

    correta: 4,

    feedbackAcerto: "Correto. Embora relacionados à inclusão, os conceitos possuem finalidades distintas na legislação.",

    feedbackErro: "Desenho universal e adaptação razoável são instrumentos diferentes de promoção da acessibilidade.",

    dicaBanca: "Desenho universal = planejamento; adaptação razoável = ajuste."
},

{
    pergunta: "Uma escola adquiriu recursos como softwares leitores de tela, teclados adaptados e dispositivos de comunicação alternativa para ampliar a participação dos estudantes com deficiência. Esses recursos constituem exemplos de:",

    alternativas: [
        "Barreiras tecnológicas.",
        "Acessibilidade arquitetônica.",
        "Adaptação curricular.",
        "Tecnologia assistiva.",
        "Atendimento especializado."
    ],

    correta: 3,

    feedbackAcerto: "Correto. A tecnologia assistiva engloba produtos, recursos, metodologias, estratégias e serviços que promovem autonomia e participação da pessoa com deficiência.",

    feedbackErro: "A tecnologia assistiva possui conceito amplo e não se limita a equipamentos eletrônicos.",

    dicaBanca: "Tecnologia assistiva = autonomia e independência."
},

{
    pergunta: "Ao discutir políticas de inclusão, um gestor afirmou que a tecnologia assistiva se limita ao uso de computadores e equipamentos eletrônicos. Considerando a Lei Brasileira de Inclusão, assinale a alternativa correta:",

    alternativas: [
        "A afirmação está correta.",
        "Tecnologia assistiva refere-se apenas a equipamentos digitais.",
        "Tecnologia assistiva compreende recursos, serviços, metodologias e estratégias destinados a ampliar funcionalidade e participação.",
        "Tecnologia assistiva aplica-se apenas à educação.",
        "Tecnologia assistiva é utilizada exclusivamente por pessoas com deficiência física."
    ],

    correta: 2,

    feedbackAcerto: "Correto. A legislação adota conceito amplo de tecnologia assistiva, abrangendo recursos e serviços voltados à autonomia e inclusão.",

    feedbackErro: "A tecnologia assistiva não se restringe a equipamentos eletrônicos ou ambientes educacionais.",

    dicaBanca: "Tecnologia assistiva possui conceito amplo na LBI."
},
{
    pergunta: "Uma escola particular recusou a matrícula de uma criança com deficiência alegando não possuir estrutura adequada para atendê-la. Os responsáveis foram orientados a procurar uma instituição especializada. À luz da Lei Brasileira de Inclusão, assinale a alternativa correta:",

    alternativas: [
        "A escola pode recusar a matrícula quando não possuir profissionais especializados.",
        "A recusa é permitida desde que a família seja encaminhada para outra instituição.",
        "A recusa da matrícula viola o sistema educacional inclusivo assegurado pela legislação.",
        "A escola pode limitar matrículas de estudantes com deficiência por razões pedagógicas.",
        "A matrícula depende da comprovação de viabilidade financeira da instituição."
    ],

    correta: 2,

    feedbackAcerto: "Correto. A LBI assegura sistema educacional inclusivo e veda práticas que impeçam o acesso da pessoa com deficiência à educação.",

    feedbackErro: "A deficiência não pode ser utilizada como justificativa para impedir matrícula ou permanência no sistema de ensino.",

    dicaBanca: "Recusa de matrícula costuma indicar ilegalidade."
},

{
    pergunta: "Durante reunião pedagógica, um gestor afirmou que estudantes com deficiência devem ser matriculados preferencialmente em instituições especializadas para receberem atendimento mais adequado. Considerando a LBI, assinale a alternativa correta:",

    alternativas: [
        "A legislação prioriza a inclusão da pessoa com deficiência no sistema regular de ensino.",
        "A segregação educacional é a regra prevista na legislação.",
        "A matrícula em classes comuns depende da inexistência de escolas especializadas.",
        "O estudante com deficiência deve frequentar exclusivamente ambientes especializados.",
        "A escolha da escola depende exclusivamente da administração pública."
    ],

    correta: 0,

    feedbackAcerto: "Correto. A LBI assegura sistema educacional inclusivo em todos os níveis, favorecendo a participação do estudante na rede regular de ensino.",

    feedbackErro: "A legislação busca promover inclusão e convivência, e não segregação.",

    dicaBanca: "Inclusão não significa separar."
},

{
    pergunta: "Uma rede de ensino decidiu substituir a frequência dos estudantes com deficiência às aulas regulares pelo Atendimento Educacional Especializado (AEE), entendendo que essa medida ampliaria o suporte pedagógico. À luz da LBI, essa decisão é:",

    alternativas: [
        "Correta, pois o AEE substitui a escolarização comum.",
        "Correta apenas para deficiência intelectual.",
        "Correta quando houver autorização da família.",
        "Incorreta, pois o AEE possui caráter complementar ou suplementar ao ensino regular.",
        "Incorreta apenas para estudantes com deficiência física."
    ],

    correta: 3,

    feedbackAcerto: "Correto. O Atendimento Educacional Especializado complementa ou suplementa a formação do estudante, não substituindo a escolarização regular.",

    feedbackErro: "Uma confusão comum é considerar que o AEE substitui as aulas regulares, o que contraria a legislação.",

    dicaBanca: "AEE complementa; não substitui."
},

{
    pergunta: "Um estudante necessita de auxílio permanente para alimentação, higiene pessoal e locomoção durante o período escolar. Para atender essa necessidade, a instituição designou um profissional específico. Nos termos da LBI, esse profissional é denominado:",

    alternativas: [
        "Professor de inclusão.",
        "Profissional de apoio escolar.",
        "Monitor pedagógico.",
        "Tutor educacional especializado.",
        "Assistente de aprendizagem."
    ],

    correta: 1,

    feedbackAcerto: "Correto. A LBI utiliza a expressão 'profissional de apoio escolar' para atividades relacionadas à alimentação, higiene e locomoção.",

    feedbackErro: "A legislação possui nomenclatura específica para essa função.",

    dicaBanca: "Decore o termo exato da lei."
},

{
    pergunta: "Durante uma capacitação, um servidor afirmou que o profissional de apoio escolar pode substituir o professor da turma quando houver estudante com deficiência. Considerando a LBI, assinale a alternativa correta:",

    alternativas: [
        "A afirmação está correta, pois o profissional de apoio assume funções pedagógicas.",
        "A afirmação está correta apenas na educação infantil.",
        "O profissional de apoio substitui o professor sempre que necessário.",
        "A substituição depende da autorização da direção escolar.",
        "A afirmação está incorreta, pois o profissional de apoio não substitui as atribuições pedagógicas do professor."
    ],

    correta: 4,

    feedbackAcerto: "Correto. O profissional de apoio auxilia na autonomia e participação do estudante, sem assumir as funções docentes.",

    feedbackErro: "Uma pegadinha recorrente é atribuir ao profissional de apoio responsabilidades pedagógicas que pertencem ao professor.",

    dicaBanca: "Profissional de apoio ≠ professor."
},{
    pergunta: "Uma unidade de saúde passou a exigir que toda pessoa com deficiência estivesse acompanhada por familiar para receber informações sobre seu tratamento. Considerando a Lei Brasileira de Inclusão, assinale a alternativa correta:",

    alternativas: [
        "A autonomia da pessoa com deficiência deve ser respeitada, não sendo admissíveis restrições generalizadas.",
        "A exigência é obrigatória para garantir segurança jurídica.",
        "A deficiência pressupõe incapacidade para compreender informações médicas.",
        "A presença de acompanhante é requisito para qualquer atendimento em saúde.",
        "A medida é válida sempre que houver deficiência intelectual."
    ],

    correta: 0,

    feedbackAcerto: "Correto. A LBI reconhece a autonomia da pessoa com deficiência e combate práticas que limitem indevidamente sua participação nas decisões relacionadas à própria saúde.",

    feedbackErro: "A deficiência não elimina automaticamente a capacidade de compreender informações ou tomar decisões.",

    dicaBanca: "Autonomia é um dos pilares da LBI."
},

{
    pergunta: "Durante uma consulta, um profissional afirmou que apenas familiares poderiam autorizar procedimentos médicos envolvendo pessoa com deficiência. À luz da LBI, essa afirmação é:",

    alternativas: [
        "Correta em qualquer situação.",
        "Correta sempre que houver deficiência física.",
        "Correta quando a pessoa possuir qualquer limitação funcional.",
        "Incorreta apenas para maiores de idade.",
        "Incorreta, pois a deficiência não retira automaticamente a capacidade de decisão da pessoa."
    ],

    correta: 4,

    feedbackAcerto: "Correto. A LBI afasta a ideia de incapacidade automática decorrente da deficiência e protege a autodeterminação da pessoa.",

    feedbackErro: "A existência de deficiência não autoriza a substituição automática da vontade da pessoa.",

    dicaBanca: "Deficiência não significa incapacidade."
},

{
    pergunta: "Um município criou programa destinado ao desenvolvimento de habilidades, ampliação da independência e fortalecimento da participação social das pessoas com deficiência. Essa política pública está diretamente relacionada à:",

    alternativas: [
        "Curatela assistencial.",
        "Segregação protetiva.",
        "Avaliação patrimonial.",
        "Habilitação e reabilitação.",
        "Tutela institucional."
    ],

    correta: 3,

    feedbackAcerto: "Correto. A habilitação e a reabilitação visam desenvolver potencialidades, autonomia, independência e participação social.",

    feedbackErro: "A legislação associa habilitação e reabilitação à promoção da autonomia e inclusão.",

    dicaBanca: "Habilitação e reabilitação = desenvolvimento da autonomia."
},

{
    pergunta: "Durante um debate sobre políticas públicas, um gestor afirmou que os serviços de reabilitação devem concentrar-se exclusivamente na recuperação clínica da pessoa com deficiência. Considerando a LBI, assinale a alternativa correta:",

    alternativas: [
        "A afirmação está correta.",
        "A reabilitação deve considerar aspectos físicos, sociais, educacionais, profissionais e comunitários.",
        "A reabilitação limita-se à recuperação física e médica.",
        "A reabilitação possui finalidade exclusivamente terapêutica.",
        "A legislação não trata da reabilitação."
    ],

    correta: 1,

    feedbackAcerto: "Correto. A LBI adota visão ampla da reabilitação, voltada à participação plena da pessoa em diversos contextos da vida social.",

    feedbackErro: "A legislação supera a visão exclusivamente médica e considera múltiplas dimensões da inclusão.",

    dicaBanca: "A abordagem da LBI é integral, não apenas clínica."
},

{
    pergunta: "Uma pessoa com deficiência visual relatou dificuldades para compreender orientações médicas porque o hospital não disponibilizava informações em formatos acessíveis. À luz da LBI, assinale a alternativa correta:",

    alternativas: [
        "A acessibilidade em saúde restringe-se às instalações físicas.",
        "Hospitais somente precisam adaptar documentos mediante decisão judicial.",
        "A comunicação acessível integra o direito à saúde da pessoa com deficiência.",
        "A obrigação aplica-se exclusivamente a hospitais públicos.",
        "A oferta de informação acessível é facultativa."
    ],

    correta: 2,

    feedbackAcerto: "Correto. O acesso à informação e à comunicação em formatos acessíveis faz parte do direito à saúde assegurado pela LBI.",

    feedbackErro: "A acessibilidade em saúde envolve também comunicação, informação e compreensão adequada dos serviços prestados.",

    dicaBanca: "Saúde acessível vai além de rampas e elevadores."
},
{
    pergunta: "Uma empresa deixou de contratar uma candidata com deficiência após constatar que seriam necessárias adaptações no ambiente de trabalho. O setor de recursos humanos justificou a decisão alegando aumento de custos operacionais. À luz da Lei Brasileira de Inclusão, assinale a alternativa correta:",

    alternativas: [
        "A decisão é legítima quando houver impacto financeiro para a empresa.",
        "A recusa caracteriza discriminação em razão da deficiência.",
        "A contratação depende exclusivamente da existência prévia de adaptações.",
        "Empresas privadas podem restringir contratações de pessoas com deficiência.",
        "A medida é válida quando houver justificativa administrativa."
    ],

    correta: 1,

    feedbackAcerto: "Correto. A exclusão de candidato em razão da deficiência configura discriminação e afronta os princípios da igualdade de oportunidades previstos na LBI.",

    feedbackErro: "A deficiência não pode servir de fundamento para restringir acesso ao trabalho ou oportunidades profissionais.",

    dicaBanca: "Custos de adaptação não justificam discriminação."
},

{
    pergunta: "Durante um processo seletivo, uma empresa exigiu exames e documentos adicionais exclusivamente dos candidatos com deficiência, sem justificativa relacionada às atribuições do cargo. Segundo a LBI, essa conduta:",

    alternativas: [
        "É permitida para aumentar a segurança do empregador.",
        "É obrigatória em qualquer seleção de pessoas com deficiência.",
        "É válida quando prevista em regulamento interno.",
        "É recomendada para garantir inclusão adequada.",
        "Configura tratamento diferenciado potencialmente discriminatório."
    ],

    correta: 4,

    feedbackAcerto: "Correto. Exigências adicionais sem justificativa objetiva podem configurar discriminação e restrição indevida de direitos.",

    feedbackErro: "A legislação protege a igualdade de oportunidades no acesso ao trabalho.",

    dicaBanca: "Exigências extras costumam ser sinal de discriminação."
},

{
    pergunta: "Um trabalhador com deficiência foi impedido de participar de programa interno de capacitação sob a justificativa de que dificilmente alcançaria cargos de maior responsabilidade. À luz da LBI, assinale a alternativa correta:",

    alternativas: [
        "A decisão é válida por razões de eficiência administrativa.",
        "A restrição é permitida quando baseada na avaliação do gestor.",
        "A empresa pode limitar programas de capacitação para pessoas com deficiência.",
        "A conduta viola o direito à igualdade de oportunidades e desenvolvimento profissional.",
        "A medida é legítima quando não houver intenção ofensiva."
    ],

    correta: 3,

    feedbackAcerto: "Correto. A LBI assegura igualdade de oportunidades também na formação, promoção e desenvolvimento profissional.",

    feedbackErro: "A exclusão de oportunidades de crescimento profissional pode caracterizar discriminação.",

    dicaBanca: "Inclusão envolve contratação e progressão profissional."
},

{
    pergunta: "Uma organização adaptou equipamentos, mobiliário e sistemas internos para possibilitar o exercício das atividades por trabalhadores com deficiência. Essa iniciativa está diretamente relacionada ao princípio da:",

    alternativas: [
        "Acessibilidade no ambiente de trabalho.",
        "Segregação funcional.",
        "Avaliação biopsicossocial.",
        "Tomada de decisão apoiada.",
        "Curatela laboral."
    ],

    correta: 0,

    feedbackAcerto: "Correto. A acessibilidade deve estar presente também no ambiente de trabalho, permitindo participação em igualdade de condições.",

    feedbackErro: "A LBI busca eliminar barreiras que dificultem o exercício profissional da pessoa com deficiência.",

    dicaBanca: "Acessibilidade não se limita a escolas e prédios públicos."
},

{
    pergunta: "Durante reunião empresarial, um gestor afirmou que a pessoa com deficiência possui direito ao trabalho, mas não necessariamente às mesmas oportunidades de progressão e promoção oferecidas aos demais empregados. Considerando a LBI, assinale a alternativa correta:",

    alternativas: [
        "A afirmação está correta.",
        "A igualdade aplica-se apenas ao ingresso no emprego.",
        "A legislação assegura igualdade de oportunidades também quanto à carreira e crescimento profissional.",
        "A promoção profissional pode ser restringida por critérios relacionados à deficiência.",
        "A progressão funcional depende exclusivamente da ausência de deficiência."
    ],

    correta: 2,

    feedbackAcerto: "Correto. A LBI protege o acesso, a permanência e o desenvolvimento profissional em igualdade de condições.",

    feedbackErro: "A inclusão no trabalho não se limita à contratação, abrangendo também oportunidades de crescimento na carreira.",

    dicaBanca: "Igualdade no trabalho inclui contratação, permanência e promoção."
},{
    pergunta: "Carlos, pessoa com deficiência intelectual, pretende celebrar contrato de locação de um imóvel. Um familiar afirmou que a deficiência, por si só, impede a prática de atos da vida civil. À luz da Lei Brasileira de Inclusão, assinale a alternativa correta:",

    alternativas: [
        "A deficiência gera incapacidade civil absoluta.",
        "A pessoa com deficiência possui capacidade legal em igualdade de condições com as demais pessoas.",
        "A deficiência impede a celebração de contratos sem autorização judicial.",
        "A prática de atos civis depende obrigatoriamente de curador.",
        "A deficiência intelectual torna a pessoa relativamente incapaz em qualquer situação."
    ],

    correta: 1,

    feedbackAcerto: "Correto. A LBI assegura à pessoa com deficiência capacidade legal em igualdade de condições com as demais pessoas em todos os aspectos da vida civil.",

    feedbackErro: "Uma das principais mudanças promovidas pela LBI foi afastar a associação automática entre deficiência e incapacidade civil.",

    dicaBanca: "Deficiência não significa incapacidade."
},

{
    pergunta: "Durante uma palestra, um participante afirmou que toda pessoa com deficiência deve ser submetida à curatela para sua própria proteção. Considerando a Lei Brasileira de Inclusão, assinale a alternativa correta:",

    alternativas: [
        "A curatela é medida extraordinária e proporcional às necessidades da pessoa.",
        "A afirmação está correta para qualquer tipo de deficiência.",
        "A curatela é obrigatória sempre que houver deficiência intelectual.",
        "A curatela deve ser aplicada preventivamente para evitar prejuízos patrimoniais.",
        "A curatela constitui medida ordinária de proteção prevista pela LBI."
    ],

    correta: 0,

    feedbackAcerto: "Correto. A LBI estabelece que a curatela é medida protetiva extraordinária, proporcional às necessidades e aplicada pelo menor tempo possível.",

    feedbackErro: "A curatela deixou de ser regra e passou a constituir medida excepcional.",

    dicaBanca: "Curatela = exceção, não regra."
},

{
    pergunta: "Juliana possui deficiência e deseja tomar decisões patrimoniais importantes, mas entende que seria útil receber auxílio de pessoas de sua confiança para compreender determinadas consequências jurídicas. Segundo a LBI, o instrumento mais adequado para essa situação é:",

    alternativas: [
        "Interdição automática.",
        "Curatela permanente.",
        "Representação obrigatória por familiar.",
        "Substituição integral da vontade da pessoa.",
        "Tomada de decisão apoiada."
    ],

    correta: 4,

    feedbackAcerto: "Correto. A tomada de decisão apoiada permite que a própria pessoa escolha apoiadores para auxiliá-la na tomada de decisões, preservando sua autonomia.",

    feedbackErro: "A legislação privilegia mecanismos que fortaleçam a autonomia da pessoa com deficiência.",

    dicaBanca: "Tomada de decisão apoiada preserva a vontade da pessoa."
},

{
    pergunta: "Em uma discussão sobre direitos civis, um estudante afirmou que a curatela alcança casamento, constituição de família, exercício de direitos reprodutivos e demais escolhas existenciais da pessoa com deficiência. À luz da LBI, essa afirmação é:",

    alternativas: [
        "Correta em qualquer hipótese.",
        "Correta apenas para deficiência intelectual.",
        "Correta quando houver decisão judicial.",
        "Incorreta, pois a curatela, em regra, restringe-se aos atos patrimoniais e negociais.",
        "Incorreta apenas para pessoas maiores de idade."
    ],

    correta: 3,

    feedbackAcerto: "Correto. A LBI protege os direitos existenciais da pessoa com deficiência, limitando a curatela aos atos patrimoniais e negociais, quando necessária.",

    feedbackErro: "A curatela não representa controle total da vida da pessoa.",

    dicaBanca: "Curatela não afeta direitos existenciais."
},

{
    pergunta: "Uma instituição financeira recusou a abertura de conta bancária para uma pessoa com deficiência, alegando que ela deveria estar obrigatoriamente representada por terceiro. Considerando a Lei Brasileira de Inclusão, assinale a alternativa correta:",

    alternativas: [
        "A exigência é válida sempre que houver deficiência intelectual.",
        "A abertura de conta depende obrigatoriamente de curador.",
        "A recusa pode configurar discriminação e violação da capacidade legal assegurada pela LBI.",
        "A instituição financeira possui autonomia para restringir serviços a pessoas com deficiência.",
        "A medida é legítima para proteger o patrimônio do interessado."
    ],

    correta: 2,

    feedbackAcerto: "Correto. A LBI assegura capacidade legal em igualdade de condições, vedando restrições automáticas baseadas exclusivamente na deficiência.",

    feedbackErro: "A deficiência não autoriza limitações generalizadas ao exercício da capacidade civil.",

    dicaBanca: "Capacidade legal é a regra; restrição é exceção."
},
{
    pergunta: "Uma repartição pública passou a disponibilizar informações exclusivamente em formato impresso comum, impossibilitando o acesso de pessoas com deficiência visual ao conteúdo divulgado. À luz da Lei Brasileira de Inclusão, assinale a alternativa correta:",

    alternativas: [
        "A medida é válida quando as informações também estiverem disponíveis presencialmente.",
        "A adaptação somente é exigida mediante decisão judicial.",
        "A obrigação aplica-se apenas aos órgãos federais.",
        "A acessibilidade à informação constitui direito assegurado à pessoa com deficiência.",
        "A disponibilização em formato acessível é facultativa."
    ],

    correta: 3,

    feedbackAcerto: "Correto. A LBI assegura o acesso à informação e à comunicação em formatos acessíveis, garantindo igualdade de oportunidades.",

    feedbackErro: "O acesso à informação acessível é um direito da pessoa com deficiência e não uma faculdade da administração.",

    dicaBanca: "Informação acessível também é acessibilidade."
},

{
    pergunta: "Durante um evento cultural, os organizadores deixaram de disponibilizar recursos de acessibilidade comunicacional para pessoas com deficiência auditiva, alegando que a maioria do público não necessitaria desses recursos. Considerando a LBI, assinale a alternativa correta:",

    alternativas: [
        "A decisão é válida quando a maioria do público não possui deficiência.",
        "A ausência de recursos acessíveis pode restringir o exercício de direitos culturais pela pessoa com deficiência.",
        "A acessibilidade comunicacional é facultativa em eventos culturais.",
        "A obrigação existe apenas para eventos financiados pelo poder público.",
        "A acessibilidade comunicacional aplica-se exclusivamente ao ambiente escolar."
    ],

    correta: 1,

    feedbackAcerto: "Correto. A LBI busca garantir participação plena da pessoa com deficiência em atividades culturais e sociais.",

    feedbackErro: "A inclusão exige eliminação de barreiras também nos espaços culturais.",

    dicaBanca: "Cultura e comunicação acessível caminham juntas."
},

{
    pergunta: "Uma instituição educacional disponibilizou materiais didáticos em Braille para estudantes com deficiência visual. Essa iniciativa tem como principal objetivo:",

    alternativas: [
        "Garantir acessibilidade à comunicação e à informação.",
        "Substituir o ensino regular.",
        "Promover segregação pedagógica.",
        "Dispensar outras formas de acessibilidade.",
        "Atender exclusivamente exigências administrativas."
    ],

    correta: 0,

    feedbackAcerto: "Correto. O Braille constitui importante recurso de acessibilidade para garantir acesso à informação e ao conhecimento.",

    feedbackErro: "O objetivo principal é assegurar participação e acesso em igualdade de condições.",

    dicaBanca: "Braille está ligado à acessibilidade informacional."
},

{
    pergunta: "Durante a elaboração de políticas inclusivas, um gestor afirmou que a comunicação acessível beneficia apenas pessoas com deficiência auditiva. À luz da LBI, essa afirmação é:",

    alternativas: [
        "Correta, pois comunicação acessível é tema restrito à surdez.",
        "Correta apenas em instituições de ensino.",
        "Incorreta, pois a acessibilidade comunicacional pode beneficiar diferentes grupos de pessoas com deficiência.",
        "Incorreta apenas para pessoas com deficiência visual.",
        "Correta quando houver utilização de Libras."
    ],

    correta: 2,

    feedbackAcerto: "Correto. A comunicação acessível abrange diferentes recursos destinados a ampliar o acesso à informação e à interação social.",

    feedbackErro: "A acessibilidade comunicacional possui alcance amplo e não se limita a um único grupo.",

    dicaBanca: "Comunicação acessível é conceito abrangente."
},

{
    pergunta: "Uma pessoa com deficiência foi impedida de participar de atividade cultural porque o local não oferecia recursos mínimos de acessibilidade. Segundo os princípios da LBI, essa situação:",

    alternativas: [
        "É aceitável quando o evento possui caráter privado.",
        "É permitida se houver limitação orçamentária dos organizadores.",
        "É legítima quando o público-alvo não for composto por pessoas com deficiência.",
        "Não possui relação com acessibilidade.",
        "Representa obstáculo à participação social e ao exercício de direitos culturais."
    ],

    correta: 4,

    feedbackAcerto: "Correto. A LBI assegura participação plena da pessoa com deficiência na vida cultural, artística, esportiva e social.",

    feedbackErro: "A exclusão de atividades culturais contraria os princípios de inclusão e igualdade de oportunidades.",

    dicaBanca: "Participação cultural também é direito garantido pela LBI."
},
{
    pergunta: "Uma pessoa com deficiência adulta manifestou o desejo de residir de forma independente, contando apenas com os apoios necessários para sua autonomia. Um familiar afirmou que a legislação prioriza a institucionalização dessas pessoas em ambientes especializados. À luz da Lei Brasileira de Inclusão, assinale a alternativa correta:",

    alternativas: [
        "A institucionalização permanente é a regra prevista na legislação.",
        "A vida independente depende exclusivamente de autorização judicial.",
        "A pessoa com deficiência deve residir obrigatoriamente com familiares.",
        "A moradia independente constitui direito assegurado à pessoa com deficiência.",
        "A autonomia residencial é admitida apenas para deficiência física."
    ],

    correta: 3,

    feedbackAcerto: "Correto. A LBI assegura o direito à moradia digna e à vida independente, com os apoios necessários para o exercício da autonomia.",

    feedbackErro: "A legislação privilegia autonomia, inclusão comunitária e participação social, não a segregação institucional.",

    dicaBanca: "Vida independente é um direito expressamente protegido."
},

{
    pergunta: "Ao organizar o atendimento ao público, um órgão governamental decidiu retirar o atendimento prioritário destinado às pessoas com deficiência, argumentando que todos deveriam ser atendidos rigorosamente pela ordem de chegada. Considerando a LBI, assinale a alternativa correta:",

    alternativas: [
        "A medida promove igualdade formal e está correta.",
        "O atendimento prioritário é direito assegurado à pessoa com deficiência.",
        "O atendimento prioritário depende de autorização judicial.",
        "O atendimento prioritário existe apenas em hospitais.",
        "A prioridade somente se aplica a crianças com deficiência."
    ],

    correta: 1,

    feedbackAcerto: "Correto. A LBI garante atendimento prioritário à pessoa com deficiência em diversas situações e serviços.",

    feedbackErro: "A legislação reconhece que a igualdade material pode exigir tratamento prioritário em determinadas circunstâncias.",

    dicaBanca: "Prioridade é instrumento de inclusão, não privilégio."
},

{
    pergunta: "Durante um processo judicial, uma pessoa com deficiência encontrou dificuldades para compreender atos processuais porque não lhe foram disponibilizados recursos de acessibilidade. À luz da LBI, assinale a alternativa correta:",

    alternativas: [
        "A acessibilidade não se aplica aos procedimentos judiciais.",
        "A obrigação de acessibilidade limita-se aos prédios do Poder Judiciário.",
        "A pessoa com deficiência deve providenciar os recursos necessários por conta própria.",
        "A acessibilidade judicial depende exclusivamente da concordância do magistrado.",
        "O acesso à justiça deve ocorrer em condições de igualdade, com oferta de recursos de acessibilidade quando necessários."
    ],

    correta: 4,

    feedbackAcerto: "Correto. A LBI assegura acesso à justiça em igualdade de condições, incluindo recursos que garantam compreensão e participação efetiva.",

    feedbackErro: "O direito de acesso à justiça envolve eliminação de barreiras físicas, comunicacionais e procedimentais.",

    dicaBanca: "Acesso à justiça também exige acessibilidade."
},

{
    pergunta: "Um estabelecimento comercial recusou deliberadamente o atendimento de uma pessoa com deficiência exclusivamente em razão de sua condição. Considerando os dispositivos da LBI, assinale a alternativa correta:",

    alternativas: [
        "A situação caracteriza discriminação vedada pela legislação e pode gerar responsabilização.",
        "A conduta configura mera irregularidade administrativa.",
        "A recusa é permitida em estabelecimentos privados.",
        "A recusa somente é ilícita quando houver ofensa verbal.",
        "A discriminação depende da existência de dano patrimonial."
    ],

    correta: 0,

    feedbackAcerto: "Correto. A discriminação contra a pessoa com deficiência é expressamente vedada pela LBI e pode acarretar responsabilização legal.",

    feedbackErro: "A discriminação não se limita a ofensas verbais e pode ocorrer por restrições, exclusões ou recusas indevidas.",

    dicaBanca: "Recusar, restringir ou impedir direitos pode configurar discriminação."
},

{
    pergunta: "Ao estudar a Lei Brasileira de Inclusão, uma candidata concluiu que seu objetivo central é assegurar e promover, em condições de igualdade, o exercício dos direitos e das liberdades fundamentais da pessoa com deficiência, visando à inclusão social e à cidadania. Essa conclusão está:",

    alternativas: [
        "Incorreta, pois a LBI trata apenas de acessibilidade.",
        "Incorreta, pois a lei se limita ao ambiente escolar.",
        "Correta.",
        "Incorreta, pois a legislação possui natureza exclusivamente assistencial.",
        "Incorreta, pois a LBI regula apenas relações entre Estado e pessoa com deficiência."
    ],

    correta: 2,

    feedbackAcerto: "Correto. Esse é o objetivo central da Lei Brasileira de Inclusão, expresso logo em suas disposições iniciais.",

    feedbackErro: "A LBI possui alcance amplo e busca garantir inclusão, cidadania, autonomia, igualdade de oportunidades e participação social.",

    dicaBanca: "Questões finais costumam cobrar o objetivo geral da lei."
}];